...
public class DatabaseController {
  ...
  public Integer getPlayer(String playerName) {
    Result<Record1<Integer>> playerIds = db.select(Player.PLAYER.ID)
        .from(Player.PLAYER).where(Player.PLAYER.NAME.equal(playerName))
        .fetch();
    ...
    if (playerIds.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          "More than one ID found for player name!");
    }
  ...
  public Integer getOrInsertPlayer(String playerName) {
    // TODO: Represent player/hand data with local java objects and only update
    // the DB at the end of
    // each file. Cleaner and significantly improves performance
    if (getPlayer(playerName) == null) {
      db.insertInto(Player.PLAYER, Player.PLAYER.NAME).values(playerName)
          .execute();
  ...
  public Integer getOrInsertHand(String handTag) {
    ...
    if (handIds.isEmpty()) {
      ...
      handIds = db.select(Hand.HAND.ID).from(Hand.HAND)
          .where(Hand.HAND.TAG.equal(handTag)).fetch();
    }
  ...
  public void addToHand(int playerId, int handId) {
    ...
    db.insertInto(Play.PLAY, Play.PLAY.PLAYERID, Play.PLAY.HANDID)
        .values(playerId, handId).execute();
  }

  public void updateVPIP(int playerId, int handId) {
    db.update(Play.PLAY)
        .set(Play.PLAY.VPIP, (byte) 1)
        .where(
            Play.PLAY.PLAYERID.equal(playerId).and(
                Play.PLAY.HANDID.equal(handId))).execute();
  }

  public void updatePFR(int playerId, int handId) {
    db.update(Play.PLAY)
        .set(Play.PLAY.PFR, (byte) 1)
        .where(
            Play.PLAY.PLAYERID.equal(playerId).and(
                Play.PLAY.HANDID.equal(handId))).execute();
  }
  ...
  public int getHands(int playerId) {
    Record1<Integer> record = db.selectCount().from(Play.PLAY)
        .where(Play.PLAY.PLAYERID.equal(playerId)).fetchOne();
    return (int) record.getValue(0);
  }

  public int getHandsVPIP(int playerId) {
    Record1<Integer> record = db.selectCount().from(Play.PLAY)
        .where(Play.PLAY.PLAYERID.equal(playerId))
